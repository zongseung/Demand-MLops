...
# ===== 지점 목록 CSV 로드 =====
STATION_LIST_PATH = PROJECT_ROOT / "station_list.csv"

if STATION_LIST_PATH.exists():
    try:
        # CSV 파일 읽기 (첫 줄 헤더 중복 문제 처리)
        df_stations = pd.read_csv(STATION_LIST_PATH, encoding="utf-8-sig")
        # '지점' 컬럼이 있고, 숫자로 변환 가능한 행만 필터링
        if '지점' in df_stations.columns:
            # 숫자로 변환 가능한 지점 ID만 추출
            station_ids = df_stations['지점'].astype(str).tolist()
            # 숫자가 아닌 값(헤더 등) 제거
            station_ids = [s for s in station_ids if s.isdigit()]
            print(f"CSV에서 {len(station_ids)}개 지점 로드 완료")
        else:
            raise ValueError("'지점' 컬럼을 찾을 수 없습니다.")
    except Exception as e:
        print(f"CSV 파일 읽기 실패: {e}")
        print("하드코딩된 지점 목록을 사용합니다.")
        station_ids = [
    ...
    print(f"경고: {STATION_LIST_PATH} 파일을 찾을 수 없습니다. 하드코딩된 지점 목록을 사용합니다.")
    station_ids = [
        '90', '93', '95', '98', '99', '100', '101', '104', '105', '106', '108', '112', '114',
        '119', '121', '127', '129', '130', '131', '133', '135', '136', '137', '138', '140',
        '143', '146', '152', '155', '156', '159', '162', '165', '168', '170', '172', '174',
